...
struct CoreDataModel {
    ...
    init() {
        ...
        selectedDate = CoreDataModel.stringYearMonth(for: Date())
        notes = []
        stringDates = []
        gameOver = false
        stage = 0
...
extension CoreDataModel {
    ...
    mutating func addNote(WORD: String, TYPE: String, DEFINITION: String) {
        do {
            let curDate = CoreDataModel.stringYearMonth(for: Date())
    ...
    mutating func remove(_ note: Note) {
        ...
        if notes.isEmpty {
            guard let idx = stringDates.firstIndex(where: { $0 == CoreDataModel.stringYearMonth(for: currentDate) }) else { return }
